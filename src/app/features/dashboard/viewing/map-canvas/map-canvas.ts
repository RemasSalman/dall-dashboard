...
export class MapCanvasComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  loadAnchorsForCurrentMap() {
    this.anchorsService.getAnchorsByMap(this.mapId).then((data: Anchor[]) => {
      this.anchors = data;
    });
  }
  ...
  saveGeneratedNeighborsToFirestore() {
    console.log('Saving neighbors...');
  }
  private updateMapSize(): void {
    if (!this.mapImage) return;

    const imageRect = this.mapImage.nativeElement.getBoundingClientRect();
    const containerRect = this.mapImage.nativeElement.parentElement!.getBoundingClientRect();

    this.displayWidth = imageRect.width;
    this.displayHeight = imageRect.height;

    this.imageOffsetX = imageRect.left - containerRect.left;
    this.imageOffsetY = imageRect.top - containerRect.top;
  }

  getScaledX(x: number): number {
    return this.imageOffsetX + (x * this.displayWidth);
  }

  getScaledY(y: number): number {
    return this.imageOffsetY + (y * this.displayHeight);
  }
